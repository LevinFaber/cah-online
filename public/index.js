...
this.socket.on('chat', function(data) {
  document.querySelector('div.hehe').innerHTML += `<p>${data.from}: ${
    data.message
  }</p>`;
});
this.socket.on('allPlayers', function(data) {
  ...
  data.forEach((player) => {
    if (!players.some((x) => x.uuid === player.uuid)) {
      players.push(player);
      document.querySelector('div.playerlist').innerHTML += `<p>${
        player.name
      }: <span data-uuid="${player.uuid}">${player.points}</p>`;
    }
    document.querySelector(`span[data-uuid="${player.uuid}"]`).innerHTML =
      player.points;
  });
});
this.socket.on('roundStart', function(data) {
  ...
  document.querySelector('div.black-card').innerHTML += `<p>${
    data.blackCard.text
  }</p>`;
});
  ...
  Object.keys(data).forEach((key) => {
    if (key !== 'blackCard') {
      document.querySelector(
        'div.answers'
      ).innerHTML += `<p data-uuid="${key}">${data[key].answer}</p>`;
    }
  });
...
document
  ...
  .addEventListener('click', () => send('startRound'));

document
  .querySelector('div.new button')
  .addEventListener('click', () =>
    send('newRoom', { name: document.querySelector('input[name=Name]').value })
  );

document
  .querySelector('div.join button')
  .addEventListener('click', () =>
    send('joinRoom', { name: document.querySelector('input[name=Name]').value })
  );

document
  .querySelector('div.chat button')
  .addEventListener('click', () =>
    send('chat', { message: document.querySelector('input[name=chat]').value })
  );
